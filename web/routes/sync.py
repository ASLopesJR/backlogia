...
def sync_steam_async(mode: str):
    ...
    mode_text = "all Steam games" if mode == "all" else "missing reviews"
    job_id = create_job(JobType.STEAM_SYNC, f"Starting Steam reviews sync ({mode_text})...")
    ...
    def run_sync(job_id: str):
        try:
            ...
            update_job_progress(job_id, 0, 1, "Initializing Steam reviews sync...")

            def on_progress(current, total, message):
                update_job_progress(job_id, current, total, message)

            force = (mode == "all")
            updated, failed = sync_steam(conn, force=force, progress_callback=on_progress)

            conn.close()

            message = f"Steam reviews sync complete: {updated} updated, {failed} failed/no data"
    ...
    return {"success": True, "job_id": job_id, "message": f"Started Steam reviews sync ({mode_text})"}
